...
export function SdkPage() {
  ...
  return <main className="sdk-page">
    <header className="sdk-header"><a className="wordmark" href="/" aria-label="Invertase"><img src={`${media}842f9b5d6727dae66d91709cbc2dbf36.svg`} alt="Invertase" /></a><nav aria-label="Global"><div className="header-menu"><button type="button" aria-expanded={openMenu === 'services'} onClick={() => setOpenMenu(openMenu === 'services' ? null : 'services')}><span>Services</span><i className={`header-chevron${openMenu === 'services' ? ' is-open' : ''}`} aria-hidden="true" /></button>{openMenu === 'services' && <div className="header-dropdown"><a href="/sdk"><span className="dropdown-icon">&lt;/&gt;</span><span><strong>SDK development</strong><small>Accelerate product growth</small></span><i>›</i></a><a href="#development"><span className="dropdown-icon">↗</span><span><strong>Cross-platform development</strong><small>Embrace Flutter or React Native</small></span><i>›</i></a></div>}</div><a href="#development">Open source</a><div className="header-menu"><button type="button" aria-expanded={openMenu === 'products'} onClick={() => setOpenMenu(openMenu === 'products' ? null : 'products')}><span>Products</span><i className={`header-chevron${openMenu === 'products' ? ' is-open' : ''}`} aria-hidden="true" /></button></div><a href="#faq">Blog</a><div className="header-menu"><button type="button" aria-expanded={openMenu === 'company'} onClick={() => setOpenMenu(openMenu === 'company' ? null : 'company')}><span>Company</span><i className={`header-chevron${openMenu === 'company' ? ' is-open' : ''}`} aria-hidden="true" /></button></div></nav><div className="header-actions"><a className="github-stars" href="https://github.com/invertase" target="_blank" rel="noreferrer" aria-label="Star us on GitHub">◉ <span>33.9k</span></a><a className="header-cta" href="#footer">Get in touch</a></div></header>
